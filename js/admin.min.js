...
/**
 * Retourne la liste des fichiers ajoutés avec le sélecteur.
 *
 * "event" correspond au contenu du champ de formulaire portant l’identifiant "file-selector".
 */
function getFiles(event, order) {
...
function loadSVG() {
  // Récupération desdits fichiers depuis le sélecteur.
  const inputSelector = document.getElementById('footer_social_links_new_shape');

  inputSelector.addEventListener('change', (event) => {
      event.preventDefault();

      console.log(inputSelector.files[0]);
  });
}
