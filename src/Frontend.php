...
class Frontend extends dcNsProcess
{
    ...
    {
        $styles = OdysseySettings::value('styles') . OdysseySettings::value('global_css_custom_mini');
    ...
    public static function odysseyPostListType(): string
    {
        if (!OdysseySettings::value('content_post_list_type')) {
            return dcCore::app()->tpl->includeFile(['src' => '_entry-list-oneline.html']);
        }

        $postlist_type_allowed = ['short', 'excerpt', 'content', 'custom'];

        $postlist_type = OdysseySettings::value('content_post_list_type');
        $postlist_type = in_array($postlist_type, $postlist_type_allowed, true) ? $postlist_type : 'oneline';
        ...
        if ($postlist_type === 'custom') {
            if (OdysseySettings::value('content_post_list_custom')) {
                ...
                $postlist_tpl = '_entry-list-oneline.html';
            }
        }
